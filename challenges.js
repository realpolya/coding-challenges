...
const computeRemainder = (a, b) => {
  ...
  // remainder 0
  if (Number.isInteger(step1)) {
    return 0;
  }

  let step2 = Math.floor(step1) * b;
  let step3 = a - step2;
  return step3;
...
const range = (a, b) => {
  ...
  if (!Number.isInteger(a) || !Number.isInteger(b)) {
    return "Please plug in integers";
  }
...
console.log("=======================reverseUpcaseString===========================");

const reverseUpcaseString = (words) => {
  // if not string
  ...
  // to upper
  let newStr = newArray.join('').toUpperCase();
...
const removeEnds = (words) => {
  // check for string
  if (typeof words !== "string") {
    return "Only accepts strings";
  }
...
const charCount = a => {
  ...
  // add all the keys to the result array
  arr.forEach((b) => {
    if (!Object.keys(result).includes(b)) {
      result[b] = 0;
    }
  })

  // loop through the string, add value to the corresponding key
  arr.forEach((b) => {
...
console.log("=======================formatWithPadding===========================");

const formatWithPadding = (a, str, b) => {
  let aStr = a.toString();
  const arr = aStr.split('');

  // if length of a is larger than b
  if (aStr.length >= b) {
    console.log(aStr);
    return aStr;
  }

  let padValue = b - aStr.length;
...
const isPalindrome = a => {
  ...
  // remove blank spaces if present (or any other punctuation)
  function removePunct(str) {
    ...
    let newA = arr.join('').toLowerCase();
    return newA;
  }

  let newStr = removePunct(a);
  newStr = removePunct(newStr); // 2 times to remove punctuation following each other
  const arr = newStr.split('');

  // reverse
  arr.forEach((char) => {
    reversedP.unshift(char);
  })
  
  // to lower case
  let b = reversedP.join('').toLowerCase();
...
function mumble(str) {
  ...
  for (let i = 0; i < str.length; i++) {
      let x = strArr[i];
      let repetition = i + 1;
      const newArr = Array.from({ length: repetition }, () => x);
      if (i !== str.length - 1) {
        newArr.push('-');
      }
      result.push(...newArr);
